...
/*
1. 如果你有一个字符，你可以通过减去 '0' 来将它转换为对应的数字。

2. 字符串与数字的加法操作会将数字转换为字符串，然后将两个字符串连接起来。例如，如果你有字符串 '2' 和数字 0，'2' + 0 的结果是字符串 '20'。

3. 对一个数进行 >>> 0 操作时，它会将这个数转换为一个 32 位无符号整数。这是因为 JavaScript 的位运算符会将它们的操作数转换为 32 位整数，并且 >>> 运算符会将结果视为无符号整数。
*/